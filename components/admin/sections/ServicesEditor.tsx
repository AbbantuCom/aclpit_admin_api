        ...
        <div key={card.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 p-4 cursor-pointer" onClick={() => setExpanded(expanded === card.id ? null : card.id)}>
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-gray-800 truncate">{card.title || 'Untitled Service'}</p>
            ...
            <div className="border-t border-gray-100 p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input placeholder="bi-journal-text" value={card.icon} onChange={(e) => update(card.id, { icon: e.target.value })} className={inputClass} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Anchor (URL fragment, e.g. research)</label>
              ...
              <ImageUpload
                value={card.image}
                onChange={(url) => update(card.id, { image: url })}
                folder="services"
                label="Card Image"
              />
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Image Alt Text</label>
                <input value={card.imageAlt} onChange={(e) => update(card.id, { imageAlt: e.target.value })} className={inputClass} />
              </div>
            </div>
          )}
        </div>
